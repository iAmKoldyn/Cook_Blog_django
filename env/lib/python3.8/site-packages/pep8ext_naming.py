...
BaseASTCheck = _ASTCheckMeta(
    "BaseASTCheck", (object,), {"__doc__": "Base for AST Checks.", "err": _err}
)
...
class NamingChecker(object):
    ...
    def parse_options(cls, options):
        ...
        cls.visitors = frozenset(
            visitor
            for visitor in BaseASTCheck.all
            for code in visitor.codes
...
class FunctionNameCheck(BaseASTCheck):
    ...
    def visit_functiondef(self, node, parents, ignore=None):
        ...
        if (
            function_type == _FunctionType.FUNCTION
            and name[:2] == "__"
            and name[-2:] == "__"
        ):
...
class VariablesCheck(BaseASTCheck):
    ...
    def visit_with(self, node, parents, ignore):
        if PY2:
            for error in self._find_errors(node.optional_vars, parents, ignore):
                yield error
            return
        for item in node.items:
            for error in self._find_errors(item.optional_vars, parents, ignore):
                yield error
