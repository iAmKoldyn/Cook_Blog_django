    ...
    if x != y:
        ...
        if repr_x == repr_y:
            ...
            if not (x_attrs is None or y_attrs is None):
                diff_ = _compare_mapping(
                    x_attrs, y_attrs, context, x, "attributes ", ".%s"
                )
            if diff_:
                return diff_
            return "Both %s and %s appear as %r, but are not equal!" % (
                context.x_label or "x",
                context.y_label or "y",
                repr_x,
            )
...
def compare_with_type(x, y, context):
    ...
    for name in "x", "y":
        ...
        to_render[name] = context.label(
            name, "{0} ({1!r})".format(_short_repr(obj), type(obj))
        )
    ...
    if x_fields and y_fields:
        if x_fields == y_fields:
            return _compare_mapping(
                dict(zip(x_fields, x)), dict(zip(y_fields, y)), context, x
            )
        else:
            return compare_with_type(x, y, context)
    return compare_sequence(x, y, context)
...
def _compare_mapping(
    x, y, context, obj_for_class, prefix="", breadcrumb="[%r]", check_y_not_x=True
):
...
def compare_call(x, y, context):
    ...
    different = (
        ...
        or context.different(x_args, y_args, " args")
        or context.different(x_kw, y_kw, " kw")
    )
    if not different:
        return
...
class StatefulComparison(object):
    ...
    def name(self):
        ...
        if self.name_attrs:
            name += "(%s)" % ", ".join(
                "%s=%r" % (n, getattr(self, n)) for n in self.name_attrs
            )
        return name
...
class Comparison(StatefulComparison):
    ...
    def __init__(
        self, object_or_type, attribute_dict=None, partial=False, **attributes
    ):
    ...
    def name(self):
        ...
        if module:
            name = name + module + "."
        name += getattr(self.expected_type, "__name__", None) or repr(
            self.expected_type
        )
        return name
