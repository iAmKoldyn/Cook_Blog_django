...
class PopenBehaviour(object):
    ...
    def __init__(self, stdout=b"", stderr=b"", returncode=0, pid=1234, poll_count=3):
        self.stdout = stdout
        self.stderr = stderr
...
class MockPopenInstance(object):
    ...
    def __init__(
        ...
        if stderr == STDOUT:
            line_iterator = chain.from_iterable(
                zip_longest(
                    stdout_value.splitlines(True), stderr_value.splitlines(True)
                )
            )
    ...
    def _record(self, names, *args, **kw):
        ...
        for base_call, store in ((call, self.calls), (self.root_call, self.all_calls)):
    ...
    if PY3:
        ...
        def communicate(self, input=None, timeout=None):
            ...
            self.returncode = self.behaviour.returncode
            return (
                self.stdout and self.stdout.read(),
...
class MockPopen(object):
    ...
    def _resolve_behaviour(
        self, stdout, stderr, returncode, pid, poll_count, behaviour
    ):
        if behaviour is None:
            return PopenBehaviour(stdout, stderr, returncode, pid, poll_count)
        else:
            return behaviour
...
extend_docstring(set_command_params, [MockPopen.set_command, MockPopen.set_default])
