...
def check_raises(x=marker, y=marker, message=None, regex=None, compare=compare, **kw):
    args = []
    for value in x, y:
    ...
    try:
        ...
        if message is not None:
            ...
            if not regex.match(actual):  # pragma: no cover
                raise AssertionError("%r did not match %r" % (actual, regex.pattern))
    else:
...
class TestCompare(CompareHelper, TestCase):
    ...
    def test_object_diff(self):
        self.check_raises(
            object(), object(), "<object object at ...> != <object object at ...>"
        )
    ...
    def test_decimal_different(self):
        self.check_raises(Decimal(1), Decimal(2), "Decimal('1') != Decimal('2')")

    def test_different_with_labels(self):
        self.check_raises(
            1, 2, "1 (expected) != 2 (actual)", x_label="expected", y_label="actual"
        )
    ...
    def test_unicode_string_different(self):
        if PY2:
            ...
            expected = "'a' != b'b'"
        self.check_raises(UnicodeLiteral("a"), BytesLiteral("b"), expected)
    ...
    def test_bytes_different(self):
        if PY2:
            ...
            expected = "\n" "b'12345678901'\n" "!=\n" "b'12345678902'"
        self.check_raises(
            BytesLiteral("12345678901"), BytesLiteral("12345678902"), expected
        )
    ...
    def test_exception_diff(self):
        ...
        if PY_37_PLUS:
            self.check_raises(
                e1, e2, "ValueError('some message') != ValueError('some other message')"
            )
        else:
            self.check_raises(
                e1,
    ...
    def test_exception_diff_c_wrapper(self):
        ...
        self.check_raises(
            ...
            ).format(module=exception_module, message="" if PY_37_PLUS else ","),
        )
    ...
    def test_tuple_and_list_strict(self):
        if PY2:
            expected = "(1, 2, 3) (<type 'tuple'>) != " "[1, 2, 3] (<type 'list'>)"
        else:
            expected = "(1, 2, 3) (<class 'tuple'>) != " "[1, 2, 3] (<class 'list'>)"

        self.check_raises((1, 2, 3), [1, 2, 3], expected, strict=True)
    ...
    def test_ignore_trailing_whitespace_non_string(self):
        self.check_raises(1, "", "1 != ''", trailing_whitespace=False)
...
b
  """,
    ...
    def test_ignore_blank_lines_non_string(self):
        self.check_raises(1, "", "1 != ''", blanklines=False)
    ...
    def test_list_subclass_strict(self):
        ...
        self.check_raises(
            [call.aCall()],
            m.method_calls,
            ("[call.aCall()] (<{0} 'list'>) != [call.aCall()] " "({1})").format(
                class_type_name, call_list_repr
            ),
            strict=True,
        )
    ...
    def test_list_subclass_long_strict(self):
        ...
        self.check_raises(
            ...
            (
                ...
                "({1})"
            ).format(class_type_name, call_list_repr),
            strict=True,
        )

    def test_prefix(self):
        self.check_raises(
            1, 2, "wrong number of orders: 1 != 2", prefix="wrong number of orders"
        )
    ...
    def test_dict_with_list(self):
        self.check_raises(
            {1: "one", 2: "two"}, [1, 2], "{1: 'one', 2: 'two'} != [1, 2]"
        )

    def test_explicit_expected(self):
        self.check_raises("x", expected="y", message="'y' (expected) != 'x' (actual)")

    def test_explicit_actual(self):
        self.check_raises("x", actual="y", message="'x' (expected) != 'y' (actual)")

    def test_explicit_both(self):
        self.check_raises(
            expected="x", actual="y", message="'x' (expected) != 'y' (actual)"
        )
    ...
    def test_invalid_two_args_expected(self):
        with ShouldRaise(
            TypeError("Exactly two objects needed, you supplied: ['z', 'x', 'y']")
        ):
            compare("x", "y", expected="z")

    def test_invalid_two_args_actual(self):
        with ShouldRaise(
            TypeError("Exactly two objects needed, you supplied: ['x', 'y', 'z']")
        ):
            compare("x", "y", actual="z")

    def test_invalid_zero_args(self):
        with ShouldRaise(TypeError("Exactly two objects needed, you supplied:")):
            compare()

    def test_invalid_one_args(self):
        with ShouldRaise(TypeError("Exactly two objects needed, you supplied: ['x']")):
            compare("x")
    ...
    def test_django_orm_is_horrible_part_2(self):
        ...
        t_compare(
            self.OrmObj("some longish string"), self.OrmObj("some longish string")
        )
        t_compare(self.OrmObj(date(2016, 1, 1)), self.OrmObj(date(2016, 1, 1)))

    def test_django_orm_is_horrible_part_3(self):
        compare(expected=self.OrmObj(1), actual=self.OrmObj(1), ignore_eq=True)
    ...
    def test_call_args_different(self):
        ...
        self.check_raises(m.foo.call_args, call(2), "'call(1)' != 'call(2)'")
    ...
    def test_repr_and_attributes_equal(self):
        ...
        self.check_raises(
            Wut(), Wut(), "Both x and y appear as 'Wut', but are not equal!"
        )
    ...
    def test_regex(self):
        ...
        self.check_raises(
            ...
            "Both x and y appear as \"re.compile('"
            + "a" * 199
            + ')", but are not equal!',
        )
...
class TestCompareObject(object):
    ...
    def test_ignore_dict_context_list_param(self):
        ...
        compare(
            self.Thing(x=1, y=2, z=3),
            self.Thing(x=1, y=4, z=5),
            comparers={self.Thing: compare_thing},
...
class ConcreteBaseClass(object):
    pass
