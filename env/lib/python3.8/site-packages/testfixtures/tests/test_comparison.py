...
class TestC(TestCase):
    ...
    def test_example_partial(self):
        self.assertEqual(
            C("testfixtures.tests.test_comparison.AClass", x=1, partial=True),
            AClass(1, 2),
        )
    ...
    def test_example_dont_use_c_wrappers_on_both_sides(self):
        ...
        assert x != y
        compare_repr(
            x, "<C:{mod}.ValueError(failed)>wrong type</>".format(mod=exception_module)
        )
        compare_repr(
            y,
    ...
    def test_repr_function(self):
        compare_repr(
            C("testfixtures.tests.sample1.z"), "<C:testfixtures.tests.sample1.z>"
        )

    def test_repr_instance(self):
        compare_repr(
            C(SampleClassA("something")),
            "<C:testfixtures.tests.sample1.SampleClassA>" "args: ('something',)" "</>",
        )

    def test_repr_exception(self):
        compare_repr(
            C(ValueError("something")),
            ("<C:{0}.ValueError>args: ('something',)</>").format(exception_module),
        )
    ...
    def test_repr_failed_wrong_class(self):
        ...
        assert c != BClass(1, 2)
        compare_repr(
            c, "<C:testfixtures.tests.test_comparison.AClass(failed)>" "wrong type</>"
        )

    def test_repr_failed_all_reasons_in_one(self):
        c = C("testfixtures.tests.test_comparison.AClass", y=5, z="missing")
        assert c != AClass(1, 2)
        compare_repr(
            c,
            "\n"
    ...
    def test_repr_failed_not_in_other(self):
        c = C("testfixtures.tests.test_comparison.AClass", x=1, y=2, z=(3,))
        assert c != AClass(1, 2)
        compare_repr(
            c,
            "\n"
    ...
    def test_repr_failed_not_in_self_partial(self):
        c = C(
            "testfixtures.tests.test_comparison.AClass", x=1, y=2, z=(3,), partial=True
        )
        assert c != AClass(x=1, y=2)
        compare_repr(
            c,
    ...
    def test_repr_failed_nested(self):
        left_side = [C(AClass, x=1, y=2), C(BClass, x=C(AClass, x=1, y=2), y=C(AClass))]
    ...
    def test_repr_failed_nested_failed(self):
        left_side = [
            C(AClass, x=1, y=2),
            C(BClass, x=C(AClass, x=1, partial=True), y=C(AClass, z=2)),
        ]
    ...
    def test_first(self):
        self.assertEqual(C("testfixtures.tests.sample1.SampleClassA"), SampleClassA())
    ...
    def run_property_equal_test(self, partial):
        ...
        self.assertEqual(C(SomeClass, prop=1, partial=partial), SomeClass())
    ...
    def run_method_equal_test(self, partial):
        ...
        instance = SomeClass()
        self.assertEqual(
            C(SomeClass, method=instance.method, partial=partial), instance
        )
    ...
    def test_repr_file_different(self):
        with TempDirectory() as d:
            ...
            f.close()
        if PY3:
            c = C("io.TextIOWrapper", name=path, mode="r", closed=False, partial=True)
            assert f != c
            compare_repr(
                c,
                "\n"
    ...
    def test_file_same(self):
        ...
        if PY3:
            self.assertEqual(
                f, C("io.TextIOWrapper", name=path, mode="r", closed=True, partial=True)
            )
        else:
            self.assertEqual(f, C(file, name=path, mode="r", closed=True, partial=True))
