...
class TestMappingComparison(object):
    def test_repr(self):
        ...
        check_repr(
            m, "<MappingComparison(ordered=False, partial=False)>'a': 1, 'b': 2</>"
        )

    def test_repr_ordered(self):
        m = MappingComparison((("b", 3), ("a", 1)), ordered=True)
        check_repr(
            m, "<MappingComparison(ordered=True, partial=False)>'b': 3, 'a': 1</>"
        )
    ...
    def test_unequal_order_recursive(self):
        m = MappingComparison(
            ((("b", "x"), 3), (("b", "y"), 1)), ordered=True, recursive=True
        )
    ...
    def test_unequal_partial_ordered_some_ignored(self):
        m = MappingComparison(
            (("b", 3), ("c", 1), ("a", 1)), partial=True, ordered=True
        )
        assert m != OrderedDict(
            (
                ("b", 3),
                ("d", 4),
                ("a", 1),
    ...
    def test_everything_wrong(self):
        m = MappingComparison(
            (("a", 1), ("b", 2), ("c", 3)), ordered=True, partial=True, recursive=True
        )
    ...
    def test_partial_nothing_specified_wrong_type(self):
        ...
        assert m != []
        check_repr(
            m, "<MappingComparison(ordered=False, partial=True)(failed)>bad type</>"
        )
