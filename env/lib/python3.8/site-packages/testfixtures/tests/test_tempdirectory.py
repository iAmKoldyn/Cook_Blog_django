...
from testfixtures import TempDirectory, Replacer, ShouldRaise, compare, OutputCapture
...
class TempDirectoryTests(TestCase):
    def test_write_with_slash_at_start(self):
        with TempDirectory() as d:
            with ShouldRaise(
                ValueError("Attempt to read or write outside the temporary Directory")
            ):
    ...
    def test_follow_symlinks(self):
        with TempDirectory() as d:
            d.write("foo/bar.txt", b"x")
            os.symlink(d.getpath("foo"), d.getpath("baz"))
            d.compare(["baz/", "baz/bar.txt", "foo/", "foo/bar.txt"], followlinks=True)
