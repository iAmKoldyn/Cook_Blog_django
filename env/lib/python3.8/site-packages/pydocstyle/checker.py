...
class ConventionChecker:
            ...
            for this_check in self.checks:
                ...
                if isinstance(definition, this_check._check_for):
                    ...
                    if (ignore_inline_noqa or not skipping_all) and not decorator_skip:
                        error = this_check(self, definition, definition.docstring)
                    else:
                        error = None
        ...
        if (
            ...
            codes = {
                ...
                if definition.is_magic
                    ...
                    else (violations.D102() if not definition.is_overload else None)
                ),
                NestedFunction: violations.D103,
                Function: (
                    lambda: violations.D103() if not definition.is_overload else None
                ),
    ...
    def _check_missing_args(docstring_args, definition):
        ...
        if isinstance(definition, Function):
            ...
            if missing_args:
                yield violations.D417(", ".join(sorted(missing_args)), definition.name)
