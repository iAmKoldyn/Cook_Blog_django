...
class UnexpectedTokenError(ParseError):
    ...
    def __str__(self):
        return "Unexpected token {}, expected {}".format(self.token, self.expected_kind)
...
class Method(Function):
    ...
    def is_public(self):
        ...
        for decorator in self.decorators:
            ...
            if re(rf"^{self.name}\.").match(decorator.name):
                return False
        name_is_public = (
...
class Parser:
    ...
    def parse_dunder_all(self):
        ...
        if not is_surrounded and "," not in dunder_all_content:
            self.dunder_all_error = "Unexpected token kind in __all__: {!r}. ".format(
                self.current.kind
            )
            return
        dunder_all_content += ")"
