...
class SubscriptVisitor(base.BaseNodeVisitor):
    ...
    def _check_redundant_subscript(self, node: ast.Subscript) -> None:
        ...
        lower_ok = node.slice.lower is None or (
            not self._is_zero(node.slice.lower) and not self._is_none(node.slice.lower)
        )

        upper_ok = node.slice.upper is None or not self._is_none(node.slice.upper)

        step_ok = node.slice.step is None or (
            not self._is_one(node.slice.step) and not self._is_none(node.slice.step)
        )
