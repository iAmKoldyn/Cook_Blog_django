...
class ExtraIndentationVisitor(BaseTokenVisitor):
    ...
    _ignored_tokens: ClassVar[Tuple[int, ...]] = (tokenize.NEWLINE,)

    _ignored_previous_token: ClassVar[Tuple[int, ...]] = (tokenize.NL,)
...
class InconsistentComprehensionVisitor(BaseTokenVisitor):
    ...
    def _apply_async(self, token: tokenize.TokenInfo) -> None:
        ...
        is_broken = for_token.string != "for" or token.start[0] != for_token.start[0]
        if is_broken:
