...
def is_primitive(node: ast.AST) -> bool:
    ...
    if isinstance(node, (ast.Tuple, ast.List)):
        ...
        return len(node.elts) == 1 and isinstance(  # we do allow `{*set_items}`
            node.elts[0], ast.Starred
        )
...
def is_unary_minus(node: ast.AST) -> bool:
    ...
    if isinstance(node, ast.UnaryOp):
        return isinstance(node.op, ast.USub) and not isinstance(
            node.operand, (Constant, ast.Num)
        )
    return False
