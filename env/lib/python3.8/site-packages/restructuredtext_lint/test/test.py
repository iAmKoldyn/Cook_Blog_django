...
class TestRestructuredtextLint(TestCase):
    ...
    def test_rst_prolog_line_offset(self):
        ...
        self.assertEqual(len(errors), 1)
        self.assertIn(
            "Possible title underline, too short for the title", errors[0].message
        )
...
class TestRestructuredtextLintCLI(TestCase):
    ...
    def test_rst_lint_correct_file(self):
        ...
        raw_output = subprocess.check_output(
            (sys.executable, rst_lint_path, valid_rst), universal_newlines=True
        )
    ...
    def test_rst_lint_folder(self):
        ...
        with self.assertRaises(subprocess.CalledProcessError) as e:
            subprocess.check_output(
                (sys.executable, rst_lint_path, dir_rst), universal_newlines=True
            )
        output = str(e.exception.output)
    ...
    def test_rst_lint_missing_file(self):
        ...
        with self.assertRaises(subprocess.CalledProcessError) as e:
            subprocess.check_output(
                (sys.executable, rst_lint_path, missing_rst), universal_newlines=True
            )
        output = str(e.exception.output)
