...
def process_parentheses(token, prev_1, prev_2):
    ...
    if token.type == OPENING_BRACKET:
        is_function = previous_token and (
            (previous_token.type in CLOSE_ATOM) or (previous_token.type in FUNCTION)
        )
        if is_function:
            if prev_2.type == DEF:
    ...
    if token.type == OPENING_SQUARE_BRACKET:
        is_index_access = previous_token and (
            (previous_token.type in CLOSING) or (previous_token.type == NAMED)
        )
        if is_index_access:
...
def get_comma_errors(tokens):
    ...
    for token in tokens:
        ...
        comma_allowed = token.type in CLOSING and (
            stack[-1].comma or stack[-1].comma in TUPLE_ISH and stack[-1].n >= 1
        )

        comma_prohibited = prev_1.type == COMMA and (
            (comma_allowed and (stack[-1].comma not in TUPLE_ISH or stack[-1].n > 1))
            or stack[-1].comma == LAMBDA_EXPR
            and token.type == COLON
        )
        if comma_prohibited:
        ...
        bare_comma_prohibited = (
            token.token.type == tokenize.NEWLINE and prev_1.type == COMMA
        )
        ...
        comma_required = (
            comma_allowed
            and prev_1.type == NEW_LINE
            and prev_2.type != COMMA
            and prev_2.type not in OPENING
        )
        if comma_required:
        ...
        pop_stack = token.type in CLOSING or (
