...
class QuoteChecker(object):
    ...
    def parse_options(cls, options):
        ...
        if hasattr(options, "quotes") and options.quotes is not None:
            ...
            warnings.warn(
                "flake8-quotes has deprecated `quotes` in favor of `inline-quotes`. "
        ...
        if (
            hasattr(options, "multiline_quotes")
            and options.multiline_quotes is not None
        ):
            # cls.config = {good_single: ', good_multiline: """, bad_single: ", bad_multiline: '''}
            #   -> {good_single: ', good_multiline: ''', bad_single: ", bad_multiline: """}
        ...
        if (
            hasattr(options, "docstring_quotes")
            and options.docstring_quotes is not None
        ):
            ...
            # Otherwise (string is inline quote)
                ...
                if last_quote_char == self.config["good_single"]:
                    if not self.config["avoid_escape"] or "r" in prefix:
                        continue
                    if (
                        self.config["good_single"] in string_contents
                        and not self.config["bad_single"] in string_contents
                    ):
                        yield {
