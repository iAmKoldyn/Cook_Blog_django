...
class Option(object):
    ...
    def normalize(self, value, *normalize_args):
        ...
        if self.comma_separated_list and isinstance(value, utils.string_types):
...
class OptionManager(object):
    ...
    def generate_versions(self, format_str="%(name)s: %(version)s", join_on=", "):
        # type: (str, str) -> str
        """Generate a comma-separated list of versions of plugins."""
        return join_on.join(
            format_str % plugin._asdict() for plugin in sorted(self.registered_plugins)
        )
