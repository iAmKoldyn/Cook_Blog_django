...
class DebuggerFinder(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        if (
            ...
            and getattr(node.func, "id", None) == "breakpoint"
        ):
        ...
        debugger_method_names = list(
            chain(self.debuggers_traces_names.values(), *debuggers.values())
        )
        ...
        if is_debugger_function:
            if node.func.id in self.debuggers_traces_names.values():
                debugger_method = next(
                    item[0]
                    for item in self.debuggers_traces_names.items()
                    if item[1] == node.func.id
                )
                entry = self.debuggers_used.setdefault(
                    (node.lineno, node.col_offset), []
                )
                if debugger_method == node.func.id:
                    entry.append(
                        "{0} trace found: {1} used".format(
                            DEBUGGER_ERROR_CODE, node.func.id
                        )
                    )
                else:
                    entry.append(
                        "{0} trace found: {1} used as {2}".format(
                            DEBUGGER_ERROR_CODE, debugger_method, node.func.id
                        )
                    )

        is_debugger_attribute = (
            getattr(node.func, "attr", None) in debugger_method_names
        )
        if is_debugger_attribute:
            ...
            if caller in self.debuggers_names.values():
                entry.append(
                    "{0} trace found: {1}.{2} used".format(
                        DEBUGGER_ERROR_CODE, caller, node.func.attr
                    )
                )
            else:
                entry.append(
                    "{0} trace found: {1} used".format(
                        DEBUGGER_ERROR_CODE, node.func.attr
                    )
                )
        ...
        for name_node in node.names:
            if name_node.name in list(debuggers.keys()):
                if name_node.asname is not None:
                    ...
                    entry = self.debuggers_redefined.setdefault(
                        (node.lineno, node.col_offset), []
                    )
                    entry.append(
                ...
                # Unlike the other imports, we don't want to consider all builtin imports as worthy of flagging.
                    ...
                    entry = self.debuggers_imported.setdefault(
                        (node.lineno, node.col_offset), []
                    )
                    entry.append(
                        "{0} import for {1} found".format(
            ...
            for name_node in node.names:
                if name_node.name in debuggers[node.module]:
                    if name_node.asname is not None:
                        ...
                        entry = self.debuggers_traces_redefined.setdefault(
                            (node.lineno, node.col_offset), []
                        )
                        entry.append(
                            "{0} import for {1} found as {2}".format(
                        ...
                        entry = self.debugger_traces_imported.setdefault(
                            (node.lineno, node.col_offset), []
                        )
                        entry.append(
                            "{0} import for {1} found".format(
